...
def parse_time_and_reason(
    ...
    if unit == "m":
        ...
        readable_time: str = f"{value} {i18n.get("minutes1", locale) if value > 1 else i18n.get("minutes2", locale)}"
    elif unit == "h":
        time_delta: timedelta = timedelta(hours=value)
        readable_time: str = f"{value} {i18n.get("hours1", locale) if value > 1 else i18n.get("hours2", locale)}"
    elif unit == "d":
        time_delta: timedelta = timedelta(days=value)
        readable_time: str = f"{value} {i18n.get("days1", locale) if value > 1 else i18n.get("days2", locale)}"
    elif unit == "w":
        time_delta: timedelta = timedelta(weeks=value)
        readable_time: str = f"{value} {i18n.get("weeks1", locale) if value > 1 else i18n.get("weeks2", locale)}"
    else:
        return None, None, None

    until_date: datetime = current_datetime + time_delta
    if not reason:
        reason = i18n.get("error-reason", locale)
